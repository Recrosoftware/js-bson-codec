...
function computeEncodingParameters(bits: number) {
  if (
    typeof bits !== "number" || !Number.isInteger(bits) || bits < 32 ||
    (bits % 32) !== 0
  ) {
    throw new Error(
      `Invalid bit size '${bits}', must be an integer multiple of 32`,
    );
  }
...
export function encodeDecimal32(
  buffer: Uint8Array,
...
export function encodeDecimal64(
  buffer: Uint8Array,
  value: number | bigint | string,
  options?: CodecOptions,
): Uint8Array {
...
export function encodeDecimal128(
  buffer: Uint8Array,
  value: number | bigint | string,
  options?: CodecOptions,
): Uint8Array {
...
export function encodeDecimal(
  ...
  for (let encodedIndex = 0; encodedIndex < bytes; ++encodedIndex) {
    const bufferIndex = littleEndian
      ? encodedIndex
      : (bytes - (encodedIndex + 1));
    buffer[bufferIndex + offset] = Number(
      (encoded >> BigInt(encodedIndex * 8)) & 0xFFn,
    );
  }
}

export function decodeDecimal32(
  buffer: Uint8Array,
  options?: CodecOptions,
): string;
export function decodeDecimal32<T>(
  buffer: Uint8Array,
  options: CodecOptions & DecimalConverter<T>,
): T;
export function decodeDecimal32<T>(
  buffer: Uint8Array,
  options?: CodecOptions & Partial<DecimalConverter<T>>,
): string | T {
  return decodeDecimal(32, buffer, options);
}

export function decodeDecimal64(
  buffer: Uint8Array,
  options?: CodecOptions,
): string;
export function decodeDecimal64<T>(
  buffer: Uint8Array,
  options: CodecOptions & DecimalConverter<T>,
): T;
export function decodeDecimal64<T>(
...
export function decodeDecimal128(
  buffer: Uint8Array,
  options?: CodecOptions,
): string;
export function decodeDecimal128<T>(
  buffer: Uint8Array,
  options: CodecOptions & DecimalConverter<T>,
): T;
export function decodeDecimal128<T>(
...
export function decodeDecimal<T>(
  bits: number,
  buffer: Uint8Array,
  options: CodecOptions & DecimalConverter<T>,
): T;
export function decodeDecimal<T>(
  ...
  for (let bufferIndex = 0; bufferIndex < bytes; ++bufferIndex) {
    const encodedIndex = littleEndian
      ? bufferIndex
      : (bytes - (bufferIndex + 1));
