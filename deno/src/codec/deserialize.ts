...
export function deserializeEntries(
  ...
  if (objectLength < 0) throw new Error(`Invalid data length '${objectLength}'`);
  ...
  if (reader.position !== (finalPosition - 1)) throw new Error(`Unexpected amount of bytes read from buffer`);

  const [zero] = readUint8(reader);
  if (zero !== 0) throw new Error(`Invalid buffer terminator`);
